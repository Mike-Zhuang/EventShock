...
| R-013 | 包容个人投资者后，情景解释被误当作个性化建议或预测 | 用户把情景内方向性讨论当作确定涨跌、目标价、个性化仓位/时点或收益保证；忽略模型条件和区间 | High | Critical | 明确主要用户与使用边界、Synthetic Badge、情景条件、区间、Limitations、证据引用，以及结果与来源事实分层 | 保留可用的情景内回答，但移除或阻断个性化指令与确定性保证；重做关键页面边界并跨个人/专业用户重新执行理解度、伤害与信任测试 | Product Lead | 产品已面向个人投资者、分析人员、研究者和教育者统一开放，但相关外部理解度与误用证据仍为 `PENDING_HUMAN_EVIDENCE` |
