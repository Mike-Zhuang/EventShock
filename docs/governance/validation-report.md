...
| 完整后端回归 | `python -m pytest` | 202 passed, 1 upstream deprecation warning | 覆盖当前自动化契约；不等于外部科学有效性 |
...
此前发现的 `runScenario(cognitiveSignals=...)` 签名不一致和上传原文通过候选 claim 回显的问题已经修复，并进入当前完整回归。最新的 202 项后端测试全部通过；本报告不再把这两项列为开放失败。
