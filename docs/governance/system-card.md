...
系统回答的是：在明确的数据、Agent、市场机制和参数假设下，一个干预怎样改变模型内部结果。系统不回答真实资产未来价格、买卖方向、交易时点或目标价，不建立现实世界因果关系，也不承诺任何收益或避损结果。

## 预期用户

- 主要用户：资产管理机构、银行和交易所中，在受控环境评估事件传播机制的市场事件风险分析人员与研究团队。
- 教学用户：课程中的教师、学生，以及学习 Human-in-the-loop、point-in-time、matched-seed 和模型治理的人员。

个人投资者不是本系统的投资决策目标用户。是否适合机构生产风险流程仍需真实目标用户访谈、独立领域审查和外部验证，当前状态为 `PENDING_HUMAN_EVIDENCE`；本卡不声称这些工作已经完成。

## 禁止用途

- 真实证券自动交易、订单路由、资产配置或风险限额执行。
- “买入、卖出、持有、交易时点、目标价、胜率”形式的投资建议。
...
任何结果都必须同时展示“情景分析而非预测或投资建议”的说明，并明确其不能回答真实资产买卖方向、交易时点、目标价、现实因果或收益结果。历史事件案例可作为机制研究输入，但在独立校准、样本外检验和外部复算完成前不得称为历史有效性验证。
