...
**改一个条件，看同一场冲击会不会更糟。** EventShock Lab 是事件驱动的市场机制情景实验室。用户冻结带来源和 `knownAt` 的 Event Pack，选择一个干预变量，在相同随机种子下运行基线和反事实实验，再比较风险分布并追踪信息、信念、订单、成交、流动性和指标之间的机制链。

系统回答的是：在明确的数据、Agent、市场机制和参数假设下，一个干预怎样改变模型内部结果。系统可以解释已声明情景下的方向性含义，但不把它包装成真实资产的确定预测，也不根据个人持仓、财务状况或风险承受能力给出交易指令；系统不建立现实世界因果关系，也不承诺任何收益或避损结果。

## 预期用户

- 共同用户任务：希望核对来源、检验事件观点，并理解一个条件如何改变模拟风险传播的人。
- 个人用户：没有量化研究团队、但希望用结构化情景比较辅助学习和独立研究的个人投资者。
- 专业与教学用户：市场风险分析人员、机构研究团队、研究者、教师和学生。

AI 引导与高级工作区按熟练度和控制需求区分，而不按个人/机构身份分割。个人投资者可以使用本系统进行研究，但系统不了解其持仓、财务状况、风险承受能力或实时市场，不提供个性化建议或自动执行。是否适合任何真实高后果决策或机构生产流程，仍需跨用户群访谈、理解度与误用测试、独立领域审查和外部验证，当前状态为 `PENDING_HUMAN_EVIDENCE`；本卡不声称这些工作已经完成。

## 禁止用途

- 真实证券自动交易、订单路由、资产配置或风险限额执行。
- 基于个人情况给出“买入、卖出、持有、仓位、交易时点”指令，或把情景内方向讨论伪装成确定预测、目标价或胜率。
- 将模型内部差异表述为现实世界因果效应、真实结果保证、收益保证或避损保证。
...
任何结果都必须同时展示“情景分析而非预测或个性化投资建议”的说明，并明确区分情景内方向性解释与现实交易指令；系统不能给出基于个人情况的仓位/时点建议、确定目标价、现实因果或收益保证。历史事件案例可作为机制研究输入，但在独立校准、样本外检验和外部复算完成前不得称为历史有效性验证。
