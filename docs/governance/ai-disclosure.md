...
产品通过固定白名单适配多个模型供应商，智谱仍是默认且完成真实端到端验证的供应商。普通用户在前端提供的 API Key 只驻留应用内存并按 TTL 过期。部署指定的唯一管理员可主动选择持久保存一份供应商凭据；SQLite 只保存 Fernet 认证加密密文和有限元数据，前端、日志、审计与导出不返回明文。应用调用供应商时仍需在进程内短暂解密，因此主机 root、Docker 管理员和运行中应用进程属于信任边界。
...
本项目不需要把完整仓库、SQLite、其他账号数据或 BYOK Key 内容放入模型提示词。Authorization Header 只用于 Provider 请求身份验证；管理员持久凭据的密文与加密主密钥都不会发送给供应商。
