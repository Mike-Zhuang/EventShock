...
- 普通用户的完整 API Key 只保存在应用进程内存，具有 TTL，并在退出、过期、供应商切换、主动清除或进程重启后删除。
- 只有 `EVENTSHOCK_ADMIN_EMAIL` 指定的管理员可明确选择服务器持久保存；其他管理员角色和普通用户没有该权限。
- 管理员持久凭据由独立主密钥使用 Fernet 认证加密，供应商、模型、参数和 API Key 均位于加密信封内；`auth_persistent_llm_credentials` 的明文字段只保存所有者、加密版本、主密钥标识和时间戳。主密钥只通过 root 管理的只读 Secret 文件挂载，不进入 SQLite、Git、镜像、Compose 环境变量或部署日志。
- API 配置视图只显示掩码和非秘密状态；任何端点都不能取回完整 Key。Dataclass、模型和异常 `repr` 必须排除 Key。
- 两种模式的明文 Key 均不得写入审计事件、实验/研究结果、AI 对话、ZIP、账户导出、异常消息或访问日志。管理员持久模式也不得写入浏览器持久存储。
- 管理员保存、替换和删除必须绑定指定账号、重新验证当前密码，并同时接受账号与来源地址每小时 5 次的限流；审计只留下不含密文、明文或 Key 提示的最小动作，账户删除必须级联删除密文。
- 主密钥缺失、格式错误、所有者不匹配或密文认证失败时必须失败关闭，不得尝试弱回退。
- 用户应使用可撤销、最小权限、可查看用量的专用供应商 Key。
- Demo 结束或怀疑泄漏时，用户应立即在界面清除配置并在供应商控制台吊销 Key。

内存存储和 Fernet 静态加密都不是专用硬件 Secret Vault。数据库单独泄露不应直接暴露管理员明文 Key，但 Host root、Docker 管理员、能够读取主密钥的运维人员、内存转储、调试器、运行中应用和恶意依赖仍可能访问它。主机、备份分离、主密钥轮换与 Crash Dump 审查为 `PENDING_HUMAN_EVIDENCE`。
...
- SQLite 文件整体没有透明加密、行级安全、独立备份账户或在线复制；只有管理员持久供应商凭据字段使用应用层 Fernet 认证加密。
...
- Cross-session 数据可见。
- 明文 Secret 出现在日志、响应、数据库、浏览器持久存储或导出，或管理员凭据密文能在缺少独立主密钥时被解密。
