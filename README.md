...
For ordinary users, an API key is isolated to the current authenticated session and held only in server-process memory. It is removed on sign-out, expiry, provider switch, or service restart. The deployment-designated administrator has one explicit exception: that account can choose to store one provider credential as authenticated encrypted ciphertext on the server so it survives sign-out and restart. The plaintext key is never returned to browser persistence, logs, audit details, account exports, experiment exports, or other users; the administrator can replace or delete it. This is encryption at rest, not end-to-end encryption: host root users, Docker administrators, and the running application process remain trusted because the application must decrypt the key transiently for provider calls.
...
- Passwords use salted scrypt digests; verification codes and session tokens are stored only as irreversible digests. Ordinary users' provider API keys never enter the account database. If the deployment-designated administrator opts in, the database stores only authenticated ciphertext and limited metadata for that one persistent credential; plaintext remains excluded from browser storage, logs, audits, and exports.
