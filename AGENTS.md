...
- 回复、说明、文档和代码注释使用简体中文；Git 提交信息除外，统一使用英文，详见“Git 协作”。
...
- 需要进入 Git 历史的修改必须在个人功能分支中完成，并通过 Pull Request 合并。
- 用户已明确授权：完成其要求的代码、运行配置、部署配置或面向用户发布的文档修改并验证后，默认执行提交、推送、创建 Pull Request、合并和部署，不需要在每次任务中重复询问。项目负责人兼仓库所有者 `Mike-Zhuang` 明确允许其本人发起的变更在所需 CI 全部通过后自行合并并触发部署，无需其他成员正式 Review Approve；其他贡献者仍应经过团队审核。CI、生产发布与健康检查门禁不得绕过；破坏性操作、变基以及任务范围外的外部写操作仍需单独取得授权。
- **纯内部文档例外**：对于问题清单、交接记录、内部计划、分析笔记等只供本地或仓库内部阅读、不会改变网站内容、构建产物、运行行为或部署配置的 Markdown 文档，默认只创建或修改文件并完成相应校验；不得自动执行建分支、提交、推送、Pull Request、合并、CI 或服务器部署。只有用户在当前任务中明确要求发布，或该文档确实属于对外网站/发布工件时，才进入完整 Git 与部署流程。
- 发布动作必须与变更影响相称。不得为了“形式上完成流程”而让纯内部文档触发应用镜像重建和生产服务重启；Git 同步与服务器部署不是所有文件修改的默认完成证明。
- 禁止使用 `git reset --hard`、`git clean -fd`、`git checkout --` 等可能丢失用户修改的命令。
- 提交前只包含当前任务相关文件，不得顺手修改或提交无关内容。
- **所有 Git 提交信息必须使用英文**，包括标题行与正文；不得使用简体中文、拼音或中英混写。该要求同样适用于分支名、标签名、Pull Request 标题与正文。
- 提交信息沿用现有的 Conventional Commits 风格：`<type>: <描述>`，`type` 取 `feat`、`fix`、`docs`、`refactor`、`test`、`chore` 等；标题行使用祈使语气、小写开头、不加句号，例如 `feat: add guided event-pack workflow`。
