...
export function ScenarioBuilderPage({ navigate }: { navigate: (view: ViewId) => void }) {
  ...
  const llmPolicy: NonNullable<ScenarioDraft['llmPolicy']> = scenario.llmPolicy ?? {
    ...
    decisionIntervalSteps: 12, callBudget: 24, maxCostUsd: 10, fallbackToRules: false,
  };
  ...
  return (
    <div className="page page--scenario">
      ...
      <section className="scenario-library" aria-labelledby="scenario-library-heading">
        ...
        <p className="scenario-library__selected-summary" title={selectedSavedScenario?.name ?? scenarioName}>
          <strong>{selectedSavedScenario?.name || scenarioName || (isZh ? '当前未保存草稿' : 'Current unsaved draft')}</strong>
          <span>{t('scenario.contentSummary', {
        ...
        <details className="scenario-library__technical">
          <summary>{isZh ? '版本、哈希与验证绑定（技术详情）' : 'Versions, hashes, and validation binding (technical details)'}</summary>
      ...
      <div className="scenario-layout">
        <div className="scenario-form">
          ...
          <section id="scenario-step-market" className="form-section scenario-step-section" tabIndex={-1}>
            ...
            <details className="scenario-advanced-group">
              <summary>{isZh ? '高级市场参数（通常保留推荐默认值）' : 'Advanced market parameters (keep recommended defaults in most cases)'}</summary>
              <p>{isZh ? '这些设置改变撮合精度、交易摩擦和保护机制。只有研究问题明确涉及市场微观结构时才建议修改。' : 'These settings change matching precision, trading frictions, and safeguards. Change them only when the research question explicitly concerns market microstructure.'}</p>
              <div className="number-grid">
          ...
          <section id="scenario-step-population" className="form-section scenario-step-section" tabIndex={-1}>
            ...
            <details className="scenario-advanced-group">
              <summary>{isZh ? '高级人口与资产负债参数' : 'Advanced population and balance-sheet parameters'}</summary>
              <p>{isZh ? '人口版本、机构占比、杠杆和卖空会改变参与者结构与风险约束。初次实验建议保留默认值。' : 'Population version, institutional share, leverage, and short selling change participant composition and risk constraints. Keep defaults for a first experiment.'}</p>
              <div className="number-grid">
            ...
            <details className="scenario-advanced-group">
              <summary>{isZh ? '高级 LLM 调度、费用与失败策略' : 'Advanced LLM scheduling, cost, and failure policy'}</summary>
              <p>{isZh ? '这些设置直接影响调用次数、费用上限和失败语义。默认禁止规则回退，避免把未成功的模型参与误报为 LLM 结果。' : 'These settings directly affect call count, cost limits, and failure semantics. Rule fallback is disabled by default so unsuccessful model participation is never mislabeled as an LLM result.'}</p>
              <div className="number-grid agent-config-grid">
          ...
          <section id="scenario-step-network" className="form-section scenario-step-section" tabIndex={-1}>
            ...
            <dl className="scenario-default-summary">
              <div><dt>{isZh ? '当前拓扑' : 'Current topology'}</dt><dd>{network.topology.replaceAll('_', ' ')}</dd></div>
              <div><dt>{isZh ? '每个节点平均连接' : 'Average connections per node'}</dt><dd>{network.averageDegree}</dd></div>
            </dl>
            <details className="scenario-advanced-group">
              <summary>{isZh ? '高级传播网络参数' : 'Advanced diffusion-network parameters'}</summary>
              <p>{isZh ? '网络设置会改变消息传播路径与更正触达。初次实验建议保留当前推荐配置；修改后应把它视为实验假设的一部分。' : 'Network settings change message paths and correction reach. Keep the recommended configuration for a first experiment; any change becomes part of the experiment assumptions.'}</p>
              <div className="number-grid">
          ...
          <section id="scenario-step-review" className="form-section scenario-step-section" tabIndex={-1}>
            ...
            <details className="scenario-advanced-group">
              <summary>{isZh ? '高级运行规模与停止规则' : 'Advanced run scale and stopping rule'}</summary>
              <p>{isZh ? '这些字段控制计算量、复现种子和何时停止收集配对结果。一般只需选择上方的配对数量和主要指标。' : 'These fields control compute, reproducibility, and when paired-result collection stops. In most cases, choose only the paired-seed count and primary outcome above.'}</p>
              <div className="number-grid review-config-grid">
