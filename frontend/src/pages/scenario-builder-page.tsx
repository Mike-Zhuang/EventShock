...
import { EmptyState, Notice, PageHeader, StatusBadge } from '../components/common';
        ...
        <div className="scenario-form">
          <section id="scenario-step-event" className="form-section scenario-step-section">
            ...
            <dl className="definition-list definition-list--compact">
              ...
              <div><dt>{isZh ? '截止时间 (asOf)' : 'Cutoff (asOf)'}</dt><dd>{eventPack.pointInTime ? new Intl.DateTimeFormat(language, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(eventPack.pointInTime)) : t('common.unavailable')}</dd></div>
              <div><dt>{isZh ? '目标证券' : 'Target instrument'}</dt><dd>{eventPack.instrument ?? market.instrumentId}</dd></div>
            </dl>
          ...
          <section id="scenario-step-market" className="form-section scenario-step-section">
            ...
            <div className="number-grid">
              <TextInput id="market-instrument" labelText={isZh ? '证券代码' : 'Instrument'} value={market.instrumentId} onChange={(event) => updateMarket({ instrumentId: event.target.value.toUpperCase() })} />
              <TextInput id="market-benchmark" labelText={isZh ? '基准' : 'Benchmark'} value={market.benchmarkId} onChange={(event) => updateMarket({ benchmarkId: event.target.value.toUpperCase() })} />
              <NumberInput id="market-initial-price" label={isZh ? '初始价格' : 'Initial price'} min={1} max={1_000_000} value={market.initialPrice} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateMarket({ initialPrice: value }); }} />
              <NumberInput id="market-tick-size" label={isZh ? '最小价格变动' : 'Tick size'} min={0.0001} max={10} step={0.01} value={market.tickSize} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateMarket({ tickSize: value }); }} />
              <NumberInput id="market-fee-bps" label={isZh ? '费用（bps）' : 'Fee (bps)'} min={0} max={100} step={0.1} value={market.feeBps} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateMarket({ feeBps: value }); }} />
              <NumberInput id="market-latency-ms" label={isZh ? '基础延迟（ms）' : 'Base latency (ms)'} min={0} max={60_000} value={market.latencyMs} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateMarket({ latencyMs: Math.round(value) }); }} />
              <NumberInput id="market-price-collar" label={isZh ? '价格保护带（bps）' : 'Price collar (bps)'} min={10} max={10_000} value={market.priceCollarBps} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateMarket({ priceCollarBps: Math.round(value) }); }} />
            </div>
            <div className="toggle-grid">
              <Toggle id="market-opening-auction" labelText={isZh ? '开盘集合竞价' : 'Opening auction'} labelA={isZh ? '关闭' : 'Off'} labelB={isZh ? '开启' : 'On'} toggled={market.openingAuction} onToggle={(value) => updateMarket({ openingAuction: value })} />
              <Toggle id="market-volatility-halt" labelText={isZh ? '波动停牌' : 'Volatility halt'} labelA={isZh ? '关闭' : 'Off'} labelB={isZh ? '开启' : 'On'} toggled={market.volatilityHalt} onToggle={(value) => updateMarket({ volatilityHalt: value })} />
            </div>
          </section>
          ...
          <section id="scenario-step-population" className="form-section scenario-step-section">
            ...
            <div className="number-grid">
              ...
              <TextInput id="population-profile" labelText={isZh ? '人口配置版本' : 'Population profile'} value={population.profileId} onChange={(event) => updatePopulation({ profileId: event.target.value })} />
              <NumberInput id="institutional-share" label={isZh ? '机构占比' : 'Institutional share'} helperText="0 - 1" value={population.institutionalShare} min={0} max={1} step={0.05} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updatePopulation({ institutionalShare: value }); }} />
              <NumberInput id="representative-llm-agents" label={isZh ? '代表性 LLM 节点' : 'Representative LLM nodes'} helperText="0 - 100" value={population.representativeLlmAgents} min={0} max={100} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updatePopulation({ representativeLlmAgents: Math.round(value) }); }} />
            </div>
            <div className="toggle-grid">
              <Toggle id="population-leverage" labelText={isZh ? '杠杆与保证金' : 'Leverage and margin'} labelA={isZh ? '关闭' : 'Off'} labelB={isZh ? '开启' : 'On'} toggled={population.leverageEnabled} onToggle={(value) => updatePopulation({ leverageEnabled: value })} />
              <Toggle id="population-short-selling" labelText={isZh ? '借券卖空' : 'Borrowed short selling'} labelA={isZh ? '关闭' : 'Off'} labelB={isZh ? '开启' : 'On'} toggled={population.shortSellingEnabled} onToggle={(value) => updatePopulation({ shortSellingEnabled: value })} />
            </div>
            <div className="subsection-heading">
              <h3>{isZh ? '认知模式与代表性 LLM 节点' : 'Cognition mode and representative LLM nodes'}</h3>
            ...
            <div className="number-grid agent-config-grid">
              <Select id="scenario-model" labelText={isZh ? '智谱模型' : 'Zhipu model'} value={llmPolicy.modelId} disabled={llmPolicy.mode === 'RULE_ONLY'} onChange={(event) => updateLlmPolicy({ modelId: event.target.value })}>
              ...
              <NumberInput id="scenario-llm-count" label={isZh ? '代表性节点数' : 'Representative node count'} min={0} max={100} value={llmPolicy.representativeAgentCount} disabled={llmPolicy.mode === 'RULE_ONLY'} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) { const count = Math.round(value); setScenario({ ...scenario, llmPolicy: { ...llmPolicy, representativeAgentCount: count }, population: { ...population, representativeLlmAgents: count } }); } }} />
              <NumberInput id="scenario-llm-interval" label={isZh ? '决策间隔（步）' : 'Decision interval (steps)'} min={1} max={100} value={llmPolicy.decisionIntervalSteps} disabled={llmPolicy.mode === 'RULE_ONLY'} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateLlmPolicy({ decisionIntervalSteps: Math.round(value) }); }} />
              <NumberInput id="scenario-llm-budget" label={isZh ? '调用预算' : 'Call budget'} min={0} max={500} value={llmPolicy.callBudget} disabled={llmPolicy.mode === 'RULE_ONLY'} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateLlmPolicy({ callBudget: Math.round(value) }); }} />
              <NumberInput id="scenario-cost-budget" label={isZh ? '模型费用硬上限（USD）' : 'Model-cost hard cap (USD)'} helperText={isZh ? '最大责任上限，不是预计账单；调用后按实耗 token 结算' : 'Maximum liability, not an expected bill; actual tokens settle after'} min={0} max={100} step={0.1} value={llmPolicy.maxCostUsd} disabled={llmPolicy.mode === 'RULE_ONLY'} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateLlmPolicy({ maxCostUsd: value }); }} />
            </div>
            ...
            <div className="toggle-grid">
              <Toggle id="scenario-rule-fallback" labelText={isZh ? '失败时降级为规则智能体' : 'Fall back to rules on failure'} labelA={isZh ? '禁止' : 'Disabled'} labelB={isZh ? '允许' : 'Enabled'} toggled={llmPolicy.fallbackToRules} onToggle={(value) => updateLlmPolicy({ fallbackToRules: value })} />
            </div>
          ...
          <section id="scenario-step-network" className="form-section scenario-step-section">
            ...
            <div className="number-grid">
              <Select id="network-topology" labelText={isZh ? '拓扑' : 'Topology'} value={network.topology} onChange={(event) => updateNetwork({ topology: event.target.value as typeof network.topology })}>
              ...
              <NumberInput id="network-degree" label={isZh ? '平均度' : 'Average degree'} value={network.averageDegree} min={2} max={50} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateNetwork({ averageDegree: Math.round(value) }); }} />
              <NumberInput id="network-rewiring" label={isZh ? '重连概率' : 'Rewiring probability'} value={network.rewiringProbability} min={0} max={1} step={0.01} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateNetwork({ rewiringProbability: value }); }} />
              <NumberInput id="network-echo" label={isZh ? '回音室强度' : 'Echo chamber strength'} value={network.echoChamberStrength} min={0} max={1} step={0.05} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateNetwork({ echoChamberStrength: value }); }} />
              <NumberInput id="network-correction" label={isZh ? '更正触达率' : 'Correction reach'} value={network.correctionReach} min={0} max={1} step={0.05} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateNetwork({ correctionReach: value }); }} />
            </div>
          </section>
          ...
          <section id="scenario-step-intervention" className="form-section scenario-step-section">
            ...
            <div className="number-grid intervention-values">
              ...
              <NumberInput id="intervention-value" label={t('scenario.interventionValue')} helperText={`${selectedOption.min} - ${selectedOption.max} ${selectedOption.unit}`} value={scenario.intervention.interventionValue} min={selectedOption.min} max={selectedOption.max} step={selectedOption.step} invalid={scenario.intervention.interventionValue < selectedOption.min || scenario.intervention.interventionValue > selectedOption.max || scenario.intervention.interventionValue === scenario.intervention.baselineValue} invalidText={t('scenario.invalidRange')} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) updateIntervention({ interventionValue: value }); }} />
            </div>
          </section>
          ...
          <section id="scenario-step-review" className="form-section scenario-step-section">
            ...
            <div className="number-grid review-config-grid">
              <NumberInput id="simulation-steps" label={t('scenario.steps')} helperText="30 - 300" value={scenario.steps} min={30} max={300} step={1} invalid={scenario.steps < 30 || scenario.steps > 300} invalidText={t('scenario.invalidRange')} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) update({ steps: Math.round(value) }); }} />
              <NumberInput id="seed-root" label={isZh ? '根随机种子' : 'Root random seed'} helperText="1 - 2147483000" value={scenario.seedRoot ?? 2_026_070_700} min={1} max={2_147_483_000} step={1} onChange={(_event, state) => { const value = numericInputValue(state.value); if (value !== undefined) update({ seedRoot: Math.round(value) }); }} />
              <Select id="primary-outcome" labelText={isZh ? '主要结果指标' : 'Primary outcome'} value={scenario.primaryOutcome ?? 'maxSpreadBps'} onChange={(event) => update({ primaryOutcome: event.target.value, secondaryOutcomes: (scenario.secondaryOutcomes ?? []).filter((item) => item !== event.target.value) })}>
