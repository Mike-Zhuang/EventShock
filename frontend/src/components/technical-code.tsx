...
const TECHNICAL_CODE_LABELS: Record<string, [english: string, chinese: string]> = {
  BEFORE_PROVIDER_DISPATCH: ['Preparing the bounded provider request', '正在准备受约束的供应商请求'],
  ...
  CONTENT_SAFETY_REVIEW: ['Content-safety review requires individual approval', '内容安全复核要求逐条批准'],
  CREDENTIAL_PATTERN: ['The response resembled a credential', '回复疑似包含凭据格式'],
  CRITICAL: ['Critical severity', '严重级别'],
  ...
  EMAIL_ADDRESS: ['Email address detected', '检测到电子邮箱地址'],
  ...
  HOST_NOT_ALLOWLISTED: ['The source host is not on the official-domain list', '来源域名不在官方域名清单中'],
  HIGH: ['High severity', '高级别'],
  ...
  LOW: ['Low severity', '低级别'],
  LOW_CONFIDENCE: ['Confidence is below the bulk-review threshold', '置信度低于批量审核阈值'],
  MEDIUM: ['Medium severity', '中级别'],
  MODEL_AUTHENTICATION_ERROR: ['The provider rejected the active API credential', '供应商拒绝了当前 API 凭据'],
  MODEL_COST_BUDGET_EXCEEDED: ['The model cost budget was exhausted', '模型费用预算已用尽'],
  MODEL_OVERLOADED: ['The model provider is temporarily overloaded', '模型供应商暂时过载'],
  MODEL_PERMISSION_ERROR: ['This API key cannot access the selected model', '当前 API Key 无权访问所选模型'],
  MODEL_PRICING_UNAVAILABLE: ['Verified model pricing is unavailable', '模型价格尚未通过核验'],
  MODEL_QUOTA_EXHAUSTED: ['The model provider quota is exhausted', '模型供应商额度已用尽'],
  MODEL_RATE_LIMITED: ['The model provider rate limit was reached', '已触发模型供应商限流'],
  MODEL_REQUEST_INVALID: ['The provider rejected the structured request', '供应商拒绝了结构化请求'],
  ...
  MODEL_TRANSPORT_ERROR: ['The model-provider connection failed temporarily', '模型供应商连接暂时失败'],
  ...
  PROVIDER_RESPONSE_FAILED: ['No usable provider response was received', '未收到可用的供应商响应'],
  PROVIDER_RESPONSE_VALIDATED: ['Provider response received and validated', '已收到并校验供应商响应'],
  ...
  REDACT_OR_EDIT: ['Redact or edit the affected field', '遮盖或编辑受影响字段'],
  REMOVE_AND_RESUBMIT: ['Remove the high-risk content and resubmit', '删除高风险内容后重新提交'],
  REMOVE_ROTATE_AND_RESUBMIT: ['Remove the content, rotate the credential, then resubmit', '删除内容、轮换凭据后重新提交'],
  REPAIRING: ['The server was validating a repaired model response', '服务器正在校验修复后的模型回复'],
  REVIEW_EDIT_OR_REMOVE: ['Review, edit, or remove the affected field', '复核、编辑或删除受影响字段'],
  ...
  VERIFY_PUBLIC_CONTACT_OR_REDACT: ['Verify that the contact is public, or redact it', '确认联系方式公开，否则将其遮盖'],
